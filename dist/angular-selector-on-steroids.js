...
/* harmony export (immutable) */ __webpack_exports__["b"] = GET_SELECTED_ITEM_TEMPLATE;
...
class SelectorSelectedItemsComponent {
    ...
    link(scope, element, attrs) {
        ...
        const getRenderableItems = items => {
            return `${items.map((currentValue, index, array) => `${Object(__WEBPACK_IMPORTED_MODULE_0__utils__["b" /* GET_SELECTED_ITEM_TEMPLATE */])(currentValue, index, array, _parentReferences)}`).join(' ')}`;
        };
        ...
        if (scope.input) {
            ...
            _subscribers.push(scope.input.subscribe(inputData => {
                if (inputData.selectedValues && inputData.selectedValues.length) {
                    if (!_parentReferences.hasOwnProperty('groupAttr') || !_parentReferences.hasOwnProperty('getObjValue') || !_parentReferences.hasOwnProperty('unset') || !_parentReferences.hasOwnProperty('multiple') || !_parentReferences.hasOwnProperty('disabled')) {
                    ...
                    if (this.debug) {
                        Object(__WEBPACK_IMPORTED_MODULE_0__utils__["a" /* CONSOLE_LOGGER */])(this.$log, `Re-drawing selected items/ options.`);
                    }
                }
            }, error => {
                if (this.debug) {
                    Object(__WEBPACK_IMPORTED_MODULE_0__utils__["a" /* CONSOLE_LOGGER */])(this.$log, `Cannot initialize, Selector Selected Items Component!`);
                }
            }));
        }
        scope.$on('$destroy', () => {
...
class SelectorDropdownItemsComponent {
    ...
    link(scope, element, attrs) {
        ...
        const getRenderableItems = (items, highlighted) => {
            return `${items.map((currentValue, index, array) => `${GET_DROPDOWN_GROUP_TEMPLATE(currentValue, index, array)}${GET_DROPDOWN_ITEM_TEMPLATE(currentValue, index, array, highlighted)}`).join(' ')}`;
        };
        ...
        if (scope.input) {
            ...
            _subscribers.push(scope.input.subscribe(inputData => {
                if (inputData.filteredOptions && inputData.filteredOptions.length) {
                    ...
                    element[0].innerHTML = getRenderableItems(inputData.filteredOptions, inputData.highlighted);
                    if (this.debug) {
...
class SelectorComponent {
    ...
    link(scope, element, attrs, controller, transclude) {
        ...
        transclude(scope, (clone, scope) => {
            ...
            const OBSERVABLE_FOR_WINDOW_RESIZE = this.$window ? __WEBPACK_IMPORTED_MODULE_1_rxjs__["Observable"].fromEvent(this.$window, 'resze') : __WEBPACK_IMPORTED_MODULE_1_rxjs__["Observable"].empty();

...
export class SelectorDropdownItemsComponent {
  ...
  link(
    ...
    const GET_DROPDOWN_ITEM_TEMPLATE = (
      ...
      return hx`<li class="selector-option js-data-item ${cls}" id="sos-data-index-${index}">${
        boundValue
      }</li>`;
    };
    ...
    const GET_DROPDOWN_GROUP_TEMPLATE = (option: any, index: number, filteredOptions: any[]) => {
      if (_parentReferences.groupAttr) {
        ...
        if (
          (boundValue && index === 0) ||
          _parentReferences.getObjValue(filteredOptions[index - 1], _parentReferences.groupAttr) !==
            boundValue
        ) {
    ...
    const getRenderableItems = (items: Array<any>, highlighted: number) => {
      const liList = hx`${items.map((currentValue: any, index: number, array: Array<any>) => {
        return hx`<span>${GET_DROPDOWN_GROUP_TEMPLATE(
          ...
          index,
          array
        )}${GET_DROPDOWN_ITEM_TEMPLATE(currentValue, index, array, highlighted)}</span>`;
      })}`;

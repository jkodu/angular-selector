...
export class SelectorComponent {
  ...
  link(
    scope: ISelector.BaseComponent.Scope,
    ...
    transclude(scope, (clone: any, scope: ISelector.BaseComponent.Scope) => {
      ...
      _subscribers.push(
        OBSERVABLE_FOR_DOM_SELECTOR_INPUT_WRAPPER.subscribe((e: Event) => {
      ...
      const scrollToHighlighted = () => {
        ...
        if (option) {
          if (
            option.offsetTop + option.offsetHeight + marginBottom >
            dd.scrollTop + dd.offsetHeight
          ) {
            this.$timeout(() => {
              dd.scrollTop =
      ...
      scope.unset = index => {
        if (!scope.multiple) {
          ...
          scope.selectedValues.splice(
            angular.isDefined(index) ? index : scope.selectedValues.length - 1,
            1
          );
        }
        selectCtrl.$setDirty();
      ...
      const updateSelected = () => {
        if (!scope.multiple) {
          ...
          const nV =
            scope.options && scope.options.length > 0
              ? (scope.value || [])
                  .map(value => {
      ...
      scope.api.unset = value => {
        const values = !value
          ? scope.selectedValues
          : (scope.selectedValues || []).filter(option => {
              return optionEquals(option, value);
            });
        const indexes = scope.selectedValues
          .map((option, index) => {
